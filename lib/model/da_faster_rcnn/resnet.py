...
from lib.model.da_faster_rcnn.faster_rcnn import _fasterRCNN
from lib.model.da_faster_rcnn.fda import FDA_source_to_target_np
import torch
...
from lib.model.da_faster_rcnn.fda import FDA_source_to_target

from lib.model.da_faster_rcnn.DA import _InstanceDA
from lib.model.da_faster_rcnn.DA import grad_reverse
...
class resnet(_fasterRCNN):
    ...
    def forward(self, im_data, im_info, gt_boxes, num_boxes, need_backprop,
        ...
        src_in_tgt = FDA_source_to_target(im_data, tgt_im_data, L=0.09)

        src_in_tgt_base_feat = self.RCNN_base(src_in_tgt.cuda())

        # feed base feature map tp RPN to obtain rois
        self.RCNN_rpn.train()

        src_in_tgt_rois, src_in_tgt_rpn_loss_cls, src_in_tgt_rpn_loss_bbox = self.RCNN_rpn(
            src_in_tgt_base_feat, im_info, gt_boxes, num_boxes)
        rpn_loss_cls += src_in_tgt_rpn_loss_cls
        rpn_loss_bbox += src_in_tgt_rpn_loss_bbox

        # if it is training phrase, then use ground trubut bboxes for refining
        if self.training:
            src_in_tgt_roi_data = self.RCNN_proposal_target(
                src_in_tgt_rois, gt_boxes, num_boxes)
            src_in_tgt_rois, src_in_tgt_rois_label, src_in_tgt_rois_target, src_in_tgt_rois_inside_ws, src_in_tgt_rois_outside_ws = src_in_tgt_roi_data

            src_in_tgt_rois_label = Variable(
                src_in_tgt_rois_label.view(-1).long())
            src_in_tgt_rois_target = Variable(
                src_in_tgt_rois_target.view(-1,
                                            src_in_tgt_rois_target.size(2)))
            src_in_tgt_rois_inside_ws = Variable(
                src_in_tgt_rois_inside_ws.view(
                    -1, src_in_tgt_rois_inside_ws.size(2)))
            src_in_tgt_rois_outside_ws = Variable(
                src_in_tgt_rois_outside_ws.view(
                    -1, src_in_tgt_rois_outside_ws.size(2)))

        src_in_tgt_rois = Variable(src_in_tgt_rois)
        ...
        if cfg.POOLING_MODE == 'crop':
            ...
            src_in_tgt_pooled_feat = self.RCNN_roi_align(
                src_in_tgt_base_feat, src_in_tgt_rois.view(-1, 5))

        # feed pooled features to top model
        src_in_tgt_pooled_feat = self._head_to_tail(src_in_tgt_pooled_feat)

        # compute bbox offset
        src_in_tgt_bbox_pred = self.RCNN_bbox_pred(src_in_tgt_pooled_feat)
        if self.training and not self.class_agnostic:
            # select the corresponding columns according to roi labels
            src_in_tgt_bbox_pred_view = src_in_tgt_bbox_pred.view(
                src_in_tgt_bbox_pred.size(0),
                int(src_in_tgt_bbox_pred.size(1) / 4), 4)
            src_in_tgt_bbox_pred_select = torch.gather(
                src_in_tgt_bbox_pred_view, 1,
                src_in_tgt_rois_label.view(src_in_tgt_rois_label.size(0), 1,
                                           1).expand(
                                               src_in_tgt_rois_label.size(0),
                                               1, 4))
            src_in_tgt_bbox_pred = src_in_tgt_bbox_pred_select.squeeze(1)

        # compute object classification probability
        src_in_tgt_cls_score = self.RCNN_cls_score(src_in_tgt_pooled_feat)
        src_in_tgt_cls_prob = F.softmax(src_in_tgt_cls_score, 1)

        if self.training:
            # classification loss
            RCNN_loss_cls += F.cross_entropy(src_in_tgt_cls_score,
                                             src_in_tgt_rois_label)

            # bounding box regression L1 loss
            RCNN_loss_bbox += _smooth_l1_loss(src_in_tgt_bbox_pred,
                                              src_in_tgt_rois_target,
                                              src_in_tgt_rois_inside_ws,
                                              src_in_tgt_rois_outside_ws)
        """ =================== for target =========================="""
        ...
        DA_cst_loss = self.consistency_loss(instance_sigmoid,
                                            consistency_prob.detach())
        """  ************** taget loss ****************  """
        ...
        tgt_consistency_prob = tgt_consistency_prob.repeat(
            tgt_instance_sigmoid.size())

        tgt_DA_cst_loss = self.consistency_loss(tgt_instance_sigmoid,
                                                tgt_consistency_prob.detach())
